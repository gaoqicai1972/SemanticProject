...
namespace WpfApp2
{
    public class SolidworksResourcePlugin
    {
        private readonly string _basePath = @"D:\Solidworks\resources";

        public SolidworksResourcePlugin()
        {
            if (!Directory.Exists(_basePath))
                throw new DirectoryNotFoundException($"Solidworks 资源目录不存在: {_basePath}");
        }

        [KernelFunction("list_categories")]
        [Description("列出所有可用的 Solidworks 资源类别")]
        ...
        [Description("列出指定 Solidworks 类别下的所有 .md 文件")]
        ...
        [Description("在指定 Solidworks 类别中搜索文件名包含关键词的文件（不区分大小写）")]
        ...
        [Description("加载指定 Solidworks 类别和文件的完整 Markdown 内容")]
